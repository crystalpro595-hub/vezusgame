...
document.getElementById("open-uc").onclick = () => {
  openPopup("popup-uc-shop");
};

document.getElementById("close-uc").onclick = () => {
  closePopup("popup-uc-shop");
};

document.getElementById("close-uc-buy").onclick = () => {
  closePopup("popup-uc-buy");
};
...
document.querySelectorAll(".uc-card").forEach(card => {
  card.onclick = () => {
    ...
    document.getElementById("uc-summary").innerHTML = `
      Вы покупаете: <b>${selectedUC.uc} UC</b><br>
      Стоимость: <b>${selectedUC.price} VC</b>
    `;

    closePopup("popup-uc-shop");
    openPopup("popup-uc-buy");
  };
});

document.getElementById("confirm-uc-buy").onclick = async () => {
  const btn = document.getElementById("confirm-uc-buy");
  if (btn.dataset.loading === "1") return;

  const pubgId = document.getElementById("pubg-id").value.trim();
  if (!pubgId) return alert("Введите PUBG ID");
  ...
  if (!bal || bal.balance < selectedUC.price) {
    alert("Недостаточно средств");
    btn.dataset.loading = "0";
    btn.disabled = false;
    btn.innerText = "✅ Купить UC";
    return;
  }

  await supabase.from("balances")
    .update({ balance: bal.balance - selectedUC.price })
    .eq("user_id", USER_ID);
